...
class OrderCommandController extends \TYPO3\CMS\Extbase\Mvc\Controller\CommandController
{
     ...
     * !!! Does only work if the table tx_rkworder_domain_model_publication is empty !!!
     *
     * @return void
     */
    public function importPublicationsCommand()
    {
        ...
        /** @var \RKW\RkwOrder\Domain\Repository\BackendUserRepository $backendUserRepository */
        $backendUserRepository = $objectManager->get('RKW\\RkwOrder\\Domain\\Repository\\BackendUserRepository');
        ...
        if (!count($publicationRepository->findAll())) {
            ...
            foreach ($publicationList as $pages) {
                ...
                $publication = $objectManager->get('RKW\\RkwOrder\\Domain\\Model\\Publication');
                ...
                $table = 'tt_content';
                $whereClause = 'pid=' . $pages->getUid() . ' AND list_type="rkworder_rkworder" ';
                $rkwOrderPlugin = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', $table, $whereClause);

                foreach ($rkwOrderPlugin as $ttContentArray) {
                ...
                $pages->setTxRkworderPublication($publication);
                ...
                foreach ($hiddenPagesList as $hiddenPages) {
                    $hiddenPages->setTxRkworderPublication($publication);
                ...
                /** @var \RKW\RkwOrder\Domain\Model\Pages $pagesElement */
                ...
                if ($pages->getUid() == 1235) {
                    ...
                    $pagesElement->setTxRkworderPublication($publication);
